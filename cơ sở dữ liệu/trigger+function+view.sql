...
/*Bai 3 Tạo view thống kê 5 học sinh xuấy sắc nhất mỗi lớp, mỗi lớp lấy 5 em có dtb cao nhất*/
ALTER view Bai3
as
select TOP(1000) DSHS.MAHS,HO,TEN,DSHS.LOP,ROUND(((TOAN+VAN)*2+LY+HOA)/6,2) AS DTB FROM DSHS
JOIN DIEM on DSHS.MAHS=DIEM.MAHS
WHERE DSHS.MAHS IN( SELECT TOP(5) hs.MAHS FROM DSHS hs
where DSHS.LOP=hs.LOP
ORDER BY ROUND(((TOAN+VAN)*2+LY+HOA)/6,2) ASC )
ORDER BY DSHS.LOP asc,ROUND(((TOAN+VAN)*2+LY+HOA)/6,2) DESC
SELECT * FROM Bai3 