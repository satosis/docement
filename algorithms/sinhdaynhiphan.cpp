...
void TRY(char *x,int k,int n) //gia su da co x0...x[k-1] co k phan tu
{
	if(k==n) cout<<x<<"\n";
	else for(x[k]='0';x[k]<='1';x[k]++) TRY(x,k+1,n);
}
...
*/
void TRY(string x,int n) //gia su da co x0...x[k-1] co k phan tu
{
	if(x.size()==n) cout<<x<<"\n";
	else 
	{
		TRY(x+"0",n);
		if(x.size()<2 || x.substr(x.size()-2)!="10") TRY(x+"1",n);
	}
}
int main()
{
