...
class Xau
{
	...
	void TRY(int n1,int m1,char *kq)  //quaylui
	{
		if(C[n1][m1]==0) {/*printf("%s\n",z);*/ Dic[string(z)]=1; return;}
		if(C[n1][m1]==C[n1-1][m1]) TRY(n1-1,m1,kq);
		if(C[n1][m1]==C[n1][m1-1]) TRY(n1,m1-1,kq);
		if(C[n1][m1]>C[n1-1][m1] &&C[n1][m1]>C[n1][m1-1])  { *kq=x[n1]; TRY(n1-1,m1-1,kq-1);} 

};
int main(){Xau S;  S.sol();}
